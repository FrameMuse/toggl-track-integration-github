...
togglbutton.render('#partial-discussion-sidebar', { observe: true }, function (
  ...
  const org = $("#repository-container-header > div.d-flex.mb-3.px-3.px-md-4.px-lg-5 > div > h1 > span.author.flex-self-stretch > a").textContent
  const rep = $("#repository-container-header > div.d-flex.mb-3.px-3.px-md-4.px-lg-5 > div > h1 > strong > a").textContent

  let description = titleElem.textContent;
  if (numElem !== null) {
    description = org + "/" + rep + numElem.textContent + ': ' + description.trim();
  }
...
togglbutton.render('.js-project-card-details .js-comment:not(.toggl)', { observe: true }, function (
  ...
  const link = togglbutton.createTimerLink({
    className: 'github',
    description: description + "pukpuk",
    projectName: projectElem && projectElem.textContent + "pukpuk"
  });
